...
genericValidation = function (collection, docId, attributes) {
  ...
  check(attributes, {
    ...
    tags: [SaneStringMatcher],
    contacts: {
      email: Match.Optional(EmailMatcher),
      phone: [SaneStringMatcher],
      site: Match.Optional(UrlMatcher)
    }
  });
...
setUpCollection = function (collection) {
  ...
  collection.deny({
    ...
    //Supporting validating updates is too complex as it is necessary to inspect mongo muttator objects
